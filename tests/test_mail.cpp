...
TEST(CreateMail, TestCreateMail) {
    ...
    Mail *new_mail = NULL;
    EXPECT_EQ(OK, create_mail_pointer_to_struct(stream1, &new_mail));
    ASSERT_NO_THROW(delete_mail(new_mail));
    fclose(stream1);

    char input2[] = "test@mail.r1u\n";
    FILE* stream2 = fmemopen(input2, strlen(input2), "r");
    EXPECT_EQ(ERROR, create_mail_pointer_to_struct(stream2, &new_mail));
    ASSERT_NO_THROW(delete_mail(new_mail));
    fclose(stream2);

    char input3[] = "test@ma2il.ru\n";
    FILE* stream3 = fmemopen(input3, strlen(input3), "r");
    EXPECT_EQ(ERROR, create_mail_pointer_to_struct(stream3, &new_mail));
    ASSERT_NO_THROW(delete_mail(new_mail));
    fclose(stream3);

    char input4[] = "test@ma.il.ru\n";
    FILE* stream4 = fmemopen(input4, strlen(input4), "r");
    EXPECT_EQ(ERROR, create_mail_pointer_to_struct(stream4, &new_mail));
    ASSERT_NO_THROW(delete_mail(new_mail));
    fclose(stream4);
}
